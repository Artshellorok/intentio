			...
			for($i = 0; $i < count($socket_arr); $i++){
				...
				if($data !== "" && $data !== false && !empty($data)){
					if($curr["handshake"]){
					  ...
					  if($data_arr["type"] == "text"){
						  $data = $data_arr["payload"];
						  $message = $data;
						  $redis->lpush("chat", $message);
						    for($j = 0; $j < count($socket_arr); $j++){
								socket_write($socket_arr[$j]["socket"], encode($message, "text"));
							}
					  }
					  else if($data_arr["type"] == "ping"){
